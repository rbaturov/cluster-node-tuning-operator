...
func (h *handler) Apply(ctx context.Context, obj client.Object, recorder record.EventRecorder, options *components.Options) error {
	...
	err = createOrUpdateRuntimeClass(ctx, h.dataPlaneClient, mfs.RuntimeClass)
	if err != nil {
		return err
	}
...
func createOrUpdateRuntimeClass(ctx context.Context, cli client.Client, rtClass *nodev1.RuntimeClass) error {
	mutated, err := resources.GetMutatedRuntimeClass(ctx, cli, rtClass)
	if err != nil {
		return err
	}
	return cli.Update(ctx, mutated, &client.UpdateOptions{})
}
