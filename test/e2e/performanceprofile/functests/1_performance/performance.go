...
var _ = Describe("[rfe_id:27368][performance]", Ordered, func() {
	...
	Context("Pre boot tuning adjusted by tuned ", func() {
		...
		It("[test_id:31198] Should set CPU affinity kernel argument", func() {
			for _, node := range workerRTNodes {
				cmdline, err := nodeInspector.ExecCommandOnDaemon(context.TODO(), &node, []string{"cat", "/proc/cmdline"})
				Expect(err).ToNot(HaveOccurred())
		...
		It("[test_id:27081][crit:high][vendor:cnf-qe@redhat.com][level:acceptance] Should set workqueue CPU mask", func() {
			for _, node := range workerRTNodes {
				...
				workqueueMaskData, err := nodeInspector.ExecCommandOnDaemon(context.TODO(), &node, []string{"cat", "/sys/devices/virtual/workqueue/cpumask"})
				Expect(err).ToNot(HaveOccurred())
				...
				workqueueWritebackMaskData, err := nodeInspector.ExecCommandOnDaemon(context.TODO(), &node, []string{"cat", "/sys/bus/workqueue/devices/writeback/cpumask"})
				Expect(err).ToNot(HaveOccurred())
		...
		It("[test_id:32375][crit:high][vendor:cnf-qe@redhat.com][level:acceptance] initramfs should not have injected configuration", func() {
			for _, node := range workerRTNodes {
				...
				rhcosId, err := nodeInspector.ExecCommandOnDaemon(context.TODO(), &node, []string{"awk", "-F", "/", "{printf $4}", "/rootfs/proc/cmdline"})
				Expect(err).ToNot(HaveOccurred())
				initramfsImagesPath, err := nodeInspector.ExecCommandOnDaemon(context.TODO(), &node, []string{"find", filepath.Join("/rootfs/boot/ostree", string(rhcosId)), "-name", "*.img"})
				Expect(err).ToNot(HaveOccurred())
				modifiedImagePath := strings.TrimPrefix(strings.TrimSpace(string(initramfsImagesPath)), "/rootfs")
				initrd, err := nodeInspector.ExecCommandOnDaemon(context.TODO(), &node, []string{"chroot", "/rootfs", "lsinitrd", modifiedImagePath})
				Expect(err).ToNot(HaveOccurred())
		...
		It("[test_id:42400][crit:medium][vendor:cnf-qe@redhat.com][level:acceptance] stalld daemon is running as sched_fifo", func() {
			for _, node := range workerRTNodes {
				pid, err := nodeInspector.ExecCommandOnNode(context.TODO(), []string{"pidof", "stalld"}, &node)
				Expect(err).ToNot(HaveOccurred())
				Expect(pid).ToNot(BeEmpty())
				sched_tasks, err := nodeInspector.ExecCommandOnNode(context.TODO(), []string{"chrt", "-ap", pid}, &node)
				Expect(err).ToNot(HaveOccurred())
		...
		It("[test_id:42696][crit:medium][vendor:cnf-qe@redhat.com][level:acceptance] Stalld runs in higher priority than ksoftirq and rcu{c,b}", func() {
			for _, node := range workerRTNodes {
				stalld_pid, err := nodeInspector.ExecCommandOnNode(context.TODO(), []string{"pidof", "stalld"}, &node)
				Expect(err).ToNot(HaveOccurred())
				Expect(stalld_pid).ToNot(BeEmpty())
				sched_tasks, err := nodeInspector.ExecCommandOnNode(context.TODO(), []string{"chrt", "-ap", stalld_pid}, &node)
				Expect(err).ToNot(HaveOccurred())
				...
				ksoftirq_pid, err := nodeInspector.ExecCommandOnNode(context.TODO(), []string{"pgrep", "-f", "ksoftirqd", "-n"}, &node)
				Expect(err).ToNot(HaveOccurred())
				Expect(ksoftirq_pid).ToNot(BeEmpty())
				sched_tasks, err = nodeInspector.ExecCommandOnNode(context.TODO(), []string{"chrt", "-ap", ksoftirq_pid}, &node)
				Expect(err).ToNot(HaveOccurred())
				match = re.FindStringSubmatch(sched_tasks)
				...
				rcu_pid, err := nodeInspector.ExecCommandOnNode(context.TODO(), []string{"pgrep", "-f", "rcu[c,b]", "-n"}, &node)
				Expect(err).ToNot(HaveOccurred())
				Expect(rcu_pid).ToNot(BeEmpty())
				sched_tasks, err = nodeInspector.ExecCommandOnNode(context.TODO(), []string{"chrt", "-ap", rcu_pid}, &node)
				Expect(err).ToNot(HaveOccurred())
				match = re.FindStringSubmatch(sched_tasks)
	...
	Context("Using performance profile", func() {
		It("Should have system services running on the system.slice cgroup", func() {
			for _, node := range workerRTNodes {
				...
				output, err := nodeInspector.ExecCommandOnNode(context.TODO(), []string{"/bin/bash", "-c", command}, &node)
				Expect(err).ToNot(HaveOccurred())
	...
	Context("RPS configuration", func() {
		...
		It("[test_id: 59572] Check RPS Mask is applied to at least one single rx queue on all veth interface", func() {
			...
			for _, node := range workerRTNodes {
				...
				for _, vethinterface := range vethInterfaces {
					...
					devsRPS, err := nodeInspector.ExecCommandOnNode(context.TODO(), getRPSMaskCmd, &node)
					Expect(err).ToNot(HaveOccurred())
		...
		It("[test_id:55012] Should have the correct RPS configuration", func() {
			...
			for _, node := range workerRTNodes {
				...
				rpsMaskContent, err := nodeInspector.ExecCommandOnNode(context.TODO(), cmd, &node)
				...
				devsRPSContent, err := nodeInspector.ExecCommandOnNode(context.TODO(), cmd, &node)
		...
		It("[test_id:54190] Should not have RPS configuration set when realtime workload hint is explicitly set", func() {
			if profile.Spec.WorkloadHints != nil && profile.Spec.WorkloadHints.RealTime != nil &&
				...
				for _, node := range workerRTNodes {
					...
					_, err := nodeInspector.ExecCommandOnNode(context.TODO(), cmd, &node)
...
func execSysctlOnWorkers(ctx context.Context, workerNodes []corev1.Node, sysctlMap map[string]string) {
	...
	for _, node := range workerNodes {
		for param, expected := range sysctlMap {
			...
			out, err = nodeInspector.ExecCommandOnDaemon(ctx, &node, []string{"sysctl", "-n", param})
			Expect(err).ToNot(HaveOccurred())
...
func checkSchedKnobs(ctx context.Context, workerNodes []corev1.Node, schedKnobs map[string]string) {
	...
	for _, node := range workerNodes {
		for param, expected := range schedKnobs {
			...
			out, err = nodeInspector.ExecCommandOnDaemon(ctx, &node, []string{"cat", knob})
			Expect(err).ToNot(HaveOccurred())
			Expect(strings.TrimSpace(string(out))).Should(Equal(expected), "parameter %s value is not %s.", param, expected)
		}
	}
